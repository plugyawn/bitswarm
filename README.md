...
the current startup stage. For run rows, the stock file-size cell is replaced
with the run start quorum and member cap while the progress bar still uses real
startup bytes/check counts before startup completes. Once startup completes,
progress switches to the run start quorum, which defaults to `2` and is capped
by the selected worker limit. The Runs view expands each run into issued seeds
sorted by issue time, and each seed expands into pending/completed rollout rows
per machine with green/red Bootstrap table states for correct and wrong
results. Run state is also projected into AriaNg as ordinary active task rows,
so the running run appears in the normal download-manager list without exposing
fake BitTorrent metadata for run tasks.
