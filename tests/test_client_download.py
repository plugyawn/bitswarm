...
def test_tracker_peer_source_captures_validated_dns_pin(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert peer_source.pinned_ips == ("93.184.216.34",)


def test_tracker_peer_source_dedupes_ipv4_mapped_dns_pins(monkeypatch: pytest.MonkeyPatch) -> None:
    import socket
    ...
    def fake_getaddrinfo(host, *args, **kwargs):
        assert host == "peer.example"
        return [
            (socket.AF_INET6, socket.SOCK_STREAM, 0, "", ("::ffff:93.184.216.34", 0, 0, 0)),
    ...
    monkeypatch.setattr(schemas_module.socket, "getaddrinfo", fake_getaddrinfo)
    peer_source = tracker_peer_source("https://peer.example", {"p00000000"})
    assert peer_source.pinned_ips == ("93.184.216.34", "1.1.1.1")
...
async def test_pinned_network_backend_tries_next_ip_after_timeout() -> None:
    class FakeBackend:
        def __init__(self) -> None:
            self.connected_hosts: list[str] = []
        ...
        async def connect_tcp(self, host, port, timeout=None, local_address=None, socket_options=None):
            self.connected_hosts.append(host)
            if host == "93.184.216.34":
                raise httpcore.ConnectTimeout("first pin timed out")
            return object()
...
async def test_pinned_network_backend_tries_next_ip_after_tls_timeout() -> None:
    class FakeStream:
        def __init__(self, host: str, closed_hosts: list[str]) -> None:
            self.host = host
            self.closed_hosts = closed_hosts
        ...
        async def aclose(self):
            self.closed_hosts.append(self.host)

        async def start_tls(self, ssl_context, server_hostname=None, timeout=None):
            assert server_hostname == "peer.example"
            if self.host == "93.184.216.34":
                raise httpcore.ConnectTimeout("first TLS handshake timed out")
            return self
    ...
    class FakeBackend:
        def __init__(self) -> None:
            self.connected_hosts: list[str] = []
            self.closed_hosts: list[str] = []

        async def connect_tcp(self, host, port, timeout=None, local_address=None, socket_options=None):
            self.connected_hosts.append(host)
            return FakeStream(host, self.closed_hosts)
    ...
    stream = await backend.connect_tcp("peer.example", 443)
    tls_stream = await stream.start_tls(
        ssl_context=ssl.create_default_context(),
        server_hostname="peer.example",
        timeout=1.0,
    )
    assert isinstance(tls_stream, FakeStream)
    assert tls_stream.host == "1.1.1.1"
    assert fake_backend.connected_hosts == ["93.184.216.34", "1.1.1.1"]
    assert fake_backend.closed_hosts == ["93.184.216.34"]


def test_peer_source_pins_preserve_resolver_order_and_merge_duplicate_hosts() -> None:
    import bitswarm.client.downloader as downloader_module

    pins = downloader_module._peer_source_pins(
        [
            downloader_module.PeerSource(
                base_url="https://peer.example",
                piece_ids=frozenset({"p00000000"}),
                pin_host="peer.example",
                pinned_ips=("93.184.216.34", "1.1.1.1"),
            ),
            downloader_module.PeerSource(
                base_url="https://peer.example",
                piece_ids=frozenset({"p00000001"}),
                pin_host="peer.example",
                pinned_ips=("8.8.8.8", "1.1.1.1"),
            ),
        ]
    )
    assert pins == {"peer.example": ("93.184.216.34", "1.1.1.1", "8.8.8.8")}
