...
class PeerSource:
    ...
    pinned_ips: tuple[str, ...] = field(default_factory=tuple)
...
async def download_manifest(
    ...
    *,
    ...
    allow_unpinned_tracker_client: bool = False,
    ...
    pins = _peer_source_pins(peer_sources)
    if client is not None and pins and not allow_unpinned_tracker_client:
        raise ValueError(
            "custom clients bypass tracker DNS pinning; pass allow_unpinned_tracker_client=True only with an "
            "equivalent pinned transport"
        )
    http_client = client or httpx.AsyncClient(
        timeout=30.0,
        transport=PinnedDNSAsyncHTTPTransport(pins),
    )
    success = False
    try:
...
def _peer_source_pins(peer_sources: list[PeerSource]) -> dict[str, tuple[str, ...]]:
    ...
    for peer_source in peer_sources:
        if peer_source.pin_host is not None:
            ...
            candidate = peer_source.pinned_ips
            if current is not None and not candidate:
                continue
            if current is None:
                pins[peer_source.pin_host] = candidate
            else:
                pins[peer_source.pin_host] = tuple(dict.fromkeys([*current, *candidate]))
    return pins
