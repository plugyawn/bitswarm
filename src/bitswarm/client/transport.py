...
class PinnedDNSAsyncNetworkStream(AsyncNetworkStream):
    """Network stream that can fail over pinned peers during TLS startup."""

    def __init__(
        self,
        *,
        delegate: AsyncNetworkBackend,
        stream: AsyncNetworkStream,
        remaining_ips: Sequence[str],
        port: int,
        connect_timeout: float | None,
        local_address: str | None,
        socket_options: Sequence[SOCKET_OPTION] | None,
    ) -> None:
        self._delegate = delegate
        self._stream = stream
        self._remaining_ips = tuple(remaining_ips)
        self._port = port
    ...
    async def start_tls(
        ...
        remaining_ips = list(self._remaining_ips)
        while True:
            try:
                ...
                if not remaining_ips:
                    raise httpcore.ConnectError(
                        "tracker-discovered peer failed TLS on all validated DNS addresses"
                    ) from failures[-1]
                self._stream = await self._connect_next_after_tls_failure(remaining_ips, failures)

    def get_extra_info(self, info: str) -> object:
        return self._stream.get_extra_info(info)

    async def _connect_next_after_tls_failure(
        self,
        remaining_ips: list[str],
        failures: list[Exception],
    ) -> AsyncNetworkStream:
        while remaining_ips:
            pinned_ip = remaining_ips.pop(0)
            try:
...
class PinnedDNSAsyncNetworkBackend(AsyncNetworkBackend):
    ...
    async def connect_tcp(
        ...
        if pinned_ips is not None:
            ...
            failures: list[Exception] = []
            for index, pinned_ip in enumerate(pinned_ips):
                try:
                    ...
                    return PinnedDNSAsyncNetworkStream(
                        delegate=self._delegate,
                        stream=stream,
                        remaining_ips=pinned_ips[index + 1 :],
                        port=port,
                    ...
                    continue
            raise httpcore.ConnectError(
                f"tracker-discovered peer {normalized_host} failed on all validated DNS addresses"
            ) from failures[-1]
        return await self._delegate.connect_tcp(
            host,
            port,
